...
function Comment({ username, text }) {
  ...
  const handleReplySubmit = (e) => {
    e.preventDefault();
    if (replyText.trim()) {
      setReplies([...replies, { text: replyText, username: 'username' }]);
      setReplyText('');
      setIsReplying(false);
    }
  };
  ...
  return (
    <div style={{ borderRadius: '4px', margin: '10px 0', padding: '10px', color: 'black', textAlign: 'left' }}>
      ...
      <p style={{ margin: '5px 0' }}>
        <button onClick={handleReplyClick} style={styles.replyButton}>Vastaa</button>
      </p>
      ...
      {isReplying && (
        <form onSubmit={handleReplySubmit} style={{ marginTop: '10px' }}>
          ...
          />
          <button type="submit" style={styles.submitButton}>Vastaa</button>
        </form>
      )}

      {replies.length > 0 && (
        <div style={{ marginTop: '10px' }}>
          <button onClick={toggleReplies} style={styles.toggleRepliesButton}>
            {showReplies ? ' ^' : `Vastauksia (${replies.length})`}
          </button>
        </div>
      )}
      ...
      {showReplies && replies.length > 0 && (
        <div style={{ marginTop: '10px', paddingLeft: '20px' }}>
          {replies.map((reply, index) => (
            <div key={index} style={{ marginBottom: '10px' }}>
              <p style={styles.replyText}>{reply.username}</p>
              <p style={styles.replyContent}>{reply.text}</p>
            </div>
          ))}
        </div>
      )}

      <div style={styles.thumbButtons}>
        <FaThumbsUp style={styles.thumbIcon} />
        <FaThumbsDown style={styles.thumbIcon} />
      </div>
    </div>
  );
}
    ...
    <>
      ...
      <Box sx={{ display: 'flex', flexDirection: 'row', width: '100vw', minHeight: '100vh', backgroundColor: 'white' }}>
        <Box sx={{ width: '75%', display: 'flex', flexDirection: 'column', padding: '40px', alignItems: 'center', backgroundColor: 'white', overflowY: 'auto', marginTop: '20px' }}>
          ...
          {selectedSection === 'notes' && (
            <Box sx={{ width: '100%', marginTop: '20px', backgroundColor: 'white', padding: '20px', borderTop: '1px solid #ddd' }}>
              <Typography variant="h6" sx={{ marginBottom: '20px', fontWeight: 'bold', textAlign: 'left' }}>Muistiinpanot</Typography>
              ...
              {notes.map((note, index) => (
                <div key={index} style={{ marginBottom: '20px' }}>
                  <div onClick={() => handleSeekToNote(note.timestamp)}>
                    <Typography variant="body1" style={{ fontWeight: 'bold' }}>{note.header}</Typography>
